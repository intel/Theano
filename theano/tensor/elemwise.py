        ...
        for i, ipt in enumerate(inputs):
            ...
            if to_sum:
                ...
                for bcast in ipt.type.broadcastable:
                    ...
                    # close if
                # close for
                print("%s, %s" % (to_sum, rval[i]))
                if (to_sum[0] == 2 and rval[i].type.ndim is 3):
                    sr = Sum_op(dimension=2, keepdim=True)(rval[i])
                else:
...
class Sum_op(gof.Op):
    ...
    def make_node(self, *inputs):
        inp = list(map(as_tensor_variable, inputs))
        bc=[]
        if self.dimension == None:
            ...
            for i in range(inp[0].ndim):
                if i == self.dimension or inp[0].broadcastable[i] == True:
                    ...
                    bc.append(False)
            out = [TensorType(inp[0].type.dtype, bc)()]
        #if self.keepdim:
        #else:
