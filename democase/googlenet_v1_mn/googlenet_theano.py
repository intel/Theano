...
import numpy as np
from baselayers import ReluLayer, SoftmaxLayer, DropoutLayer, Conv2DLayer, MaxPool2DLayer, LocalResponseNormalization2DLayer, ConcatLayer, DenseLayer, AveragePool2DLayer

# MLSL based OP
import multinode
from multinode import collect


def aux_tower(name, input_layer,input_shape):
    net = {}
    params = []
    weight_types = []

    # input shape = (14x14x512or528)
    net['aux_tower_pool'] = AveragePool2DLayer(input_layer, input_shape, pool_size=5, stride=3, pad=0, ignore_border=False)
    output_shape = net['aux_tower_pool'].get_output_shape_for()
    
    # output shape = (4x4x512or528)
    filter_shape = (128, output_shape[1], 1, 1)
    net['aux_tower_1x1'] = Conv2DLayer(net['aux_tower_pool'].output, output_shape, filter_shape, flip_filters=False)
    ...
    net['relu'] = ReluLayer(net['FC_1'].output, output_shape)
    ...
    net['classifier'] = SoftmaxLayer(net['FC_2'].output)
...
def build_inception_module_4in(inception_name, x,y,xx,yy, input_shape, nfilters):
    ...
    net['1x1'] = Conv2DLayer(x, input_shape, filter_shape, flip_filters=False)
    ...
    net['3x3_reduce'] = Conv2DLayer(y, input_shape, filter_shape, flip_filters=False)
    ...
    net['3x3'] = Conv2DLayer(net['3x3_reduce'].output, output_shape, filter_shape, padsize=1, flip_filters=False)
    ...
    net['5x5_reduce'] = Conv2DLayer(xx, input_shape, filter_shape, flip_filters=False)
    ...
    net['5x5'] = Conv2DLayer(net['5x5_reduce'].output, output_shape, filter_shape, padsize=2, flip_filters=False)
    ...
    net['pool'] = MaxPool2DLayer(yy, input_shape, pool_size=3, stride=1, pad=1)
    output_shape = net['pool'].get_output_shape_for()
    
    filter_shape = (nfilters[0], output_shape[1], 1, 1)
    net['pool_proj'] = Conv2DLayer(net['pool'].output, output_shape, filter_shape, flip_filters=False)    
...
def build_inception_module(inception_name, input_layer, input_shape, nfilters):
    ...
    filter_shape = (nfilters[1], input_shape[1], 1, 1)
    net['1x1'] = Conv2DLayer(input_layer, input_shape, filter_shape, flip_filters=False)
    params +=  [net['1x1'].W, net['1x1'].b]
    weight_types += net['1x1'].weight_types
    output_shape = net['1x1'].get_output_shape_for()
    input_shapes[0] = output_shape
    ...
    filter_shape = (nfilters[2], input_shape[1], 1, 1)
    net['3x3_reduce'] = Conv2DLayer(input_layer, input_shape, filter_shape, flip_filters=False)
    params += [net['3x3_reduce'].W, net['3x3_reduce'].b]
    weight_types += net['3x3_reduce'].weight_types
    output_shape = net['3x3_reduce'].get_output_shape_for()
    ...
    filter_shape = (nfilters[4], input_shape[1], 1, 1)
    net['5x5_reduce'] = Conv2DLayer(input_layer, input_shape, filter_shape, flip_filters=False)
    params += [net['5x5_reduce'].W, net['5x5_reduce'].b]
    weight_types += net['5x5_reduce'].weight_types
    output_shape = net['5x5_reduce'].get_output_shape_for()
...
class googlenet(object):
    ...
    def __init__(self, input_shape):
        ...
        net['conv1/7x7_s2'] = Conv2DLayer(net['input'], input_shape, filter_shape, convstride=2, padsize=3, flip_filters=False)
        ...
        net['pool1/3x3_s2'] = MaxPool2DLayer(net['conv1/7x7_s2'].output, output_shape, pool_size=3, stride=2, ignore_border=False)
        ...
        net['conv2/3x3_reduce'] = Conv2DLayer(net['pool1/norm1'].output, output_shape, filter_shape, flip_filters=False)
        ...
        net['conv2/3x3'] = Conv2DLayer(net['conv2/3x3_reduce'].output, output_shape, filter_shape, padsize=1, flip_filters=False)
        ...
        net['pool2/3x3_s2'] = MaxPool2DLayer(net['conv2/norm2'].output, output_shape,  pool_size=3, stride=2, ignore_border=False)
        ...
        net['pool3/3x3_s2'] = MaxPool2DLayer(net['inception_3b'].output, output_shape, pool_size=3, stride=2, ignore_border=False)
        ...
        pre_params, pre_weight_types, p_y_given_x_1, negative_log_likelihood_1, classifier2 = aux_tower('loss1',net['inception_4a'].output, aux_tower_shape1)
        params += pre_params
        weight_types += pre_weight_types
        ...
        pre_params, pre_weight_types, p_y_given_x_2, negative_log_likelihood_2, classifier3 = aux_tower('loss2',net['inception_4d'].output,aux_tower_shape2)
        params += pre_params
        weight_types += pre_weight_types
        ...
        net['pool4/3x3_s2'] = MaxPool2DLayer(net['inception_4e'].output, output_shape, pool_size=3, stride=2, ignore_border=False)
        ...
        net['pool5/7x7_s1'] = AveragePool2DLayer(net['inception_5b'].output, output_shape, pool_size=7, stride=1, pad=0, ignore_border=False)
	...
	net['loss3'] = SoftmaxLayer(net['loss3/classifier'].output)
...
def compile_train_model(model, learning_rate=0.01, batch_size=256, image_size=(3, 224, 224), use_momentum=True, momentum=0.9, weight_decay=0.0002, multinode_mode=False):
